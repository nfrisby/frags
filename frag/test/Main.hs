...
importPath :: String -> DynFlags -> DynFlags
importPath path dflags = dflags{importPaths = path : importPaths dflags}
...
main = defaultErrorHandler defaultFatalMessager defaultFlushOut $ runGhc (Just libdir) $ do
  ...
  modifySessionDynFlags $
    ...
    . languageExtension KindSignatures
    . languageExtension PolyKinds
    . languageExtension RankNTypes
    . languageExtension ScopedTypeVariables
    . languageExtension TypeFamilies
    . languageExtension TypeOperators
    . plugin "Data.Frag.Plugin"
    . pluginOpts "Data.Frag.Plugin" opts
    . expose "frag"
    . (\x -> x{hscTarget = HscInterpreted,ghcLink = LinkInMemory,ghcMode = CompManager})
  getSession >>= \hsc_env -> getSessionDynFlags >>= \dflags ->
    liftIO (initializePlugins hsc_env dflags) >>= GHC.setSessionDynFlags
...
runTestDir e tk = do
  let dir = "tests" </> (case tk of IllTyped -> "ill-typed"; WellTyped -> "well-typed")
...
runGivenTest e path
  | "tests/ill-typed/" `isPrefixOf` path = proceed IllTyped
  | "tests/well-typed/" `isPrefixOf` path = proceed WellTyped
  | otherwise = bad1 $ path ++ " is not prefixed by tests/{ill,well}-typed/."
  where
  proceed tk = do
...
runTest e tk path
  ...
  modifySessionDynFlags $ foldr (.) id [ importPath (dir </> "aux") | dir <- splitPath (takeDirectory path) ]
